...
class AdminSite(object):
    ...
    def register(self,model_class,admin_class=None):
        ...
        app_name:用户每个app的名字,可以通过models.Customer._meta.app_label获取-->crm 类型为str
        model_name:用户每个app中models.py中定义的每个model名字的小写，可以通过models.Customer._meta.model_name获取 -->  customer 类型为str
        """

        #print("register",model_class,admin_class)
        app_name = model_class._meta.app_label
        model_name = model_class._meta.model_name
        # 为了避免多个model共享同一个BaseMyAdmin内存对象,为每一个model中类都实例化一次
        if not admin_class:
