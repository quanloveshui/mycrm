...
from myadmin.permission_list import perm_dic
...
def perm_check(*args,**kwargs):
    ...
    """
    reverse 通过别名获取url
    resolve 通过url获取别名,使用resolve方法，该方法返回一个ResolverMatch对象
    例如：
       rm = resolve(request.path)
       app_name = rm.app_name
       url_name = rm.url_name
    """
    current_url_name = resolve_url_obj.url_name  # 获取用当前请求url对应的url_name
    ...
    for permission_key,permission_val in  perm_dic.items():
        ...
        #依次匹配url name、请求方法、请求参数
        if per_url_name == current_url_name: #匹配到 current request url
            if per_method == request.method: #匹配到 request method
                # if not  perm_args: #if no args defined in perm dic, then set this request to passed perm

                #逐个匹配参数，每个参数时候都要能对应的上
                ...
                #开始匹配自定义权限钩子函数,此函数返回True或者False
